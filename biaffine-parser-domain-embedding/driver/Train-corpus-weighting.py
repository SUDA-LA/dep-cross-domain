...
def train(test_data,data,dev_data, parser, vocab, config):
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(one_corpus_weighting, config.train_batch_size, True):
            words, extwords, tags, heads, rels, lengths, masks,tbank_ids = \
                batch_data_variable(onebatch, vocab)
            sumLength = sum(lengths)
            parser.model.train()

            parser.forward(words, extwords, tags, masks,tbank_ids)
...
def evaluate(data, parser, vocab, outputFile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        words, extwords, tags, heads, rels, lengths, masks ,tbank_ids= \
            batch_data_variable(onebatch, vocab)
        count = 0
        arcs_batch, rels_batch = parser.parse(words, extwords, tags, lengths, masks,tbank_ids)
