...
class Vocab(object):
    PAD, ROOT, UNK = 0, 1, 2
    def __init__(self, word_counter, tag_counter, rel_counter, tbank_counter,relroot='root', min_occur_count = 2):
        self._root = relroot
        ...
        self._id2rel = ['<pad>', relroot,'pred','subj-in','frag','punc','repet']
        self._id2tbank = ['<pad>']
        ...
        for rel, count in rel_counter.most_common():
            if rel != relroot and rel not in self._id2rel: self._id2rel.append(rel)

        for tbank, count in tbank_counter.most_common():
            if tbank != relroot:self._id2tbank.append(tbank)
        ...
        self._tbank2id = reverse(self._id2tbank)
        if len(self._tbank2id) != len(self._id2tbank):
            print("serious bug: POS tags dumplicated, please check!")

        print("Vocab info: #words %d, #tags %d, #rels %d,#tbanks %d" % (self.vocab_size, self.tag_size, self.rel_size, self.tbank_size))
    ...
    def tbank2id(self, xs):
        if isinstance(xs, list):
            return [self._tbank2id.get(x) for x in xs]
        return self._tbank2id.get(xs)

    def id2tbank(self, xs):
        if isinstance(xs, list):
            return [self._id2tbank[x] for x in xs]
        return self._id2tbank[xs]
...
def creatVocab(corpusFile, min_occur_count):
    ...
    with open(corpusFile, 'r') as infile:
        for sentence in readDepTree(infile):
            for dep in sentence:
                ...
                tbank_counter[dep.treebank_id] += 1
                if dep.head == -1:  #补全的数据
                    continue
                if dep.head != 0:
    ...
    return Vocab(word_counter, tag_counter, rel_counter,tbank_counter, root, min_occur_count)
