...
def sentence2id(sentence, vocab):
    ...
    for dep in sentence:
        ...
        result.append([wordid, extwordid, tagid, head, relid,tbankid])
...
def batch_data_variable(batch, vocab):
    ...
    return words, extwords, tags, heads, rels, lengths, masks,tbank_ids

def batch_variable_depTree(trees, heads, rels, lengths, vocab):
    for tree, head, rel, length in zip(trees, heads, rels, lengths):
        sentence = []
        for idx in range(length):
            sentence.append(Dependency(idx, tree[idx].org_form, tree[idx].tag, head[idx], vocab.id2rel(rel[idx]),tree[idx].treebank_id))
        yield sentence
