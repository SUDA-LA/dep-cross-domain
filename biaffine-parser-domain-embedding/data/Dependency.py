...
class Dependency:  #以词为单位
    def __init__(self, id, form, tag, head, rel, treebank_id,proxy_tbank="BC"):
...
def readDepTree(file, vocab=None):
    ...
    else: sentence = [Dependency(0, vocab._root_form, vocab._root, -1, vocab._root,last_tbank)]
    for line in file:
        ...
        if not tok or line.strip() == '' or line.strip().startswith('#'):
            ...
            if vocab is None:
                ...
                sentence = [Dependency(0, vocab._root_form, vocab._root, -1, vocab._root,last_tbank)]
        elif len(tok) == 10:
            if tok[6] == '_': tok[6] = '-1'
            try:
                sentence.append(Dependency(int(tok[0]), tok[1], tok[3], int(tok[6]), tok[7],tok[9]))
                last_tbank = tok[9]
            except Exception:
                pass
        else:
