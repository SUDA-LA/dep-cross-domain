...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        print("Training for %s epochs and %s batches all %s n_train_tokens" % (
            options['n_epochs'], n_batches_total,n_train_tokens))
...
def load_options_latest_checkpoint(tf_save_dir):
    options_file = os.path.join(tf_save_dir, 'options.json')  # 连接地址，查找tf_save_dir/options.json
    ckpt_file = tf.train.latest_checkpoint(tf_save_dir)  # tensorflow提供的函数：自动获取最后一次保存的模型
