...
def train(test_data,data, dev_data, parser, vocab, config):
    ...
    for iter in range(config.train_iters):
        ...
        data1 = data[0:config.domain_data_size]
        data2 = data[config.domain_data_size:]
        one_corpus_weighting = data_corpus_weighting(data1, data2, config.domain_num_corpus_weighting,config.cdt_domain_ration)
        ...
        for onebatch in data_iter(one_corpus_weighting, config.train_batch_size, True):
            words, extwords, tags, heads, rels, lengths, masks, elmosens,tbank_ids = batch_data_variable(onebatch, vocab)
            sumLength = sum(lengths)
            parser.model.train()

            parser.forward(words, extwords, tags, masks,elmosens,config.train_elmo,tbank_ids)
            ...
            if batch_iter % config.validate_every == 0 or batch_iter == batch_num:
                arc_correct, rel_correct, arc_total, dev_uas, dev_las = \
                    evaluate(dev_data, parser, vocab, config.dev_file + '.' + str(global_step),config.dev_elmo)
                ...
                if dev_las > best_LAS:
                    ...
                    evaluate(test_data, parser, vocab, config.test_file + '.' + str(global_step),config.test_elmo)
...
def evaluate(data, parser, vocab, outputFile,elmofile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        words, extwords, tags, heads, rels, lengths, masks, elmosens ,tbank_ids= \
            batch_data_variable(onebatch, vocab)
        count = 0
        arcs_batch, rels_batch = parser.parse(words, extwords, tags, lengths, masks,elmosens,elmofile,tbank_ids)
...
if __name__ == '__main__':
    ...
    train(test_data,data, dev_data, parser, vocab, config)
