...
class BiaffineParser(object):
    ...
    def forward(self, words, extwords, tags, masks,elmosens,elmofile,tbank_ids):
        if self.use_cuda:
            ...
            tbank_ids=tbank_ids.cuda(self.device)

        arc_logits, rel_logits = self.model.forward(words, extwords, tags, masks,elmosens,elmofile,tbank_ids)
        # cache
    ...
    def compute_loss(self, true_arcs, true_rels, lengths):
        ...
        index_true_arcs = _model_var(self.model,pad_sequence(true_arcs, length=l1, padding=0, dtype=np.int64))
        true_arcs = _model_var(self.model,pad_sequence(true_arcs, length=l1, padding=-1, dtype=np.int64))
        masks = []
    ...
    def parse(self, words, extwords, tags, lengths, masks,elmosens,elmofile,tbank_ids):
        if words is not None:
            self.forward(words, extwords, tags, masks,elmosens,elmofile,tbank_ids)
        ROOT = self.root
        ...
        for arc_logit, rel_logit, length in zip(arc_logits, rel_logits, lengths):
            ...
            arc_pred = arc_argmax(arc_probs, length, False)
            
            rel_probs = rel_logit[np.arange(len(arc_pred)), arc_pred]
            rel_pred = rel_argmax(rel_probs, length, ROOT, False)
