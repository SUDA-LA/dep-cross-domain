...
class Dependency:  #以词为单位
    def __init__(self, id, form, tag, head, rel,charid,treebank_id,proxy_tbank="cdt"):
        ...
        self.proxy_tbank=proxy_tbank  #代理treebank_id,默认cdt


    def __str__(self):
        values = [str(self.id), self.org_form, "_", self.tag, "_", "_", str(self.head), self.rel, "_", "_", str(self.charid)]
        return '\t'.join(values)
...
def readDepTree(file, vocab=None):
    ...
    # else: sentence = [Dependency(0, vocab._root_form, vocab._root, -1, vocab._root)]
        ...
        sentence = [Dependency(0, vocab._root_form, vocab._root, -1, vocab._root,-1,'cdt')]
    for line in file:
        ...
        if not tok or line.strip() == '' or line.strip().startswith('#'):
            ...
            if tok[6] == '_': tok[6] = '-1'
            try:
                sentence.append(Dependency(int(tok[0]), tok[1], tok[3], int(tok[6]), tok[7],charid,tok[9]))
                charid += 1
            except Exception:
                pass
        else:
