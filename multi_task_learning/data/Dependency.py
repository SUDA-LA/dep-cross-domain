...
class Instance:
    def __init__(self, list_sentence, tgt_type):
        self.sentence = list_sentence
        self.tgt_type = tgt_type

    def __len__(self):
        return len(self.sentence)

def parse_my_conll(file_path):
...
def readDepTree(file,tgt_type, vocab=None):
    proj = 0
    total = 0
    min_count = 1
        ...
        if not tok or line.strip() == '' or line.strip().startswith('#'):
            if len(sentence) > min_count:  #sententce至少有一个词
                if DepTree(sentence).isProj():
                    proj += 1
                total += 1
                yield Instance(sentence, tgt_type)
            if vocab is None:
                sentence = []
            else:
