...
class BiaffineParser(object):
    ...
    def forward(self, words, extwords, tags, masks,type):
        if self.use_cuda:
            words, extwords = words.cuda(self.device), extwords.cuda(self.device),
            tags = tags.cuda(self.device)
            masks = masks.cuda(self.device)

        arc_logits, rel_logits = self.model.forward(words, extwords, tags, masks,type)
        # cache
    ...
    def parse(self, words, extwords, tags, lengths, masks,type):
        if words is not None:
            self.forward(words, extwords, tags, masks,type)
        ROOT = self.root
