...
class ParserModel(nn.Module):
    def __init__(self, vocab, config, pretrained_embedding, type_corpus, type_corpus2):
        ...
        self.type_corpus, self.type_corpus2 = type_corpus, type_corpus2
        ...
        self.mlp_arc_dep2 = NonLinear(
            input_size=2 * config.lstm_hiddens,
            hidden_size=config.mlp_arc_size + config.mlp_rel_size,
            activation=nn.LeakyReLU(0.1))
        self.mlp_arc_head2 = NonLinear(
            input_size=2 * config.lstm_hiddens,
            hidden_size=config.mlp_arc_size + config.mlp_rel_size,
            activation=nn.LeakyReLU(0.1))
        ...
        self.arc_biaffine1 = Biaffine(config.mlp_arc_size, config.mlp_arc_size, \
                                     1, bias=(True, False))
        self.rel_biaffine1 = Biaffine(config.mlp_rel_size, config.mlp_rel_size, \
                                     vocab.rel_size, bias=(True, True))
        self.arc_biaffine2 = Biaffine(config.mlp_arc_size, config.mlp_arc_size, \
                                      1, bias=(True, False))
        self.rel_biaffine2 = Biaffine(config.mlp_rel_size, config.mlp_rel_size, \
                                      vocab.rel_size, bias=(True, True))

    def forward(self, words, extwords, tags, masks,type_corpus):
        ...
        if type_corpus == self.type_corpus:
            x_all_dep = self.mlp_arc_dep1(outputs)
            x_all_head = self.mlp_arc_head1(outputs)
            ...
            arc_logit = self.arc_biaffine1(x_arc_dep, x_arc_head)
            ...
            rel_logit_cond = self.rel_biaffine1(x_rel_dep, x_rel_head)
        elif type_corpus == self.type_corpus2:
            x_all_dep = self.mlp_arc_dep2(outputs)
            x_all_head = self.mlp_arc_head2(outputs)
            ...
            arc_logit = self.arc_biaffine2(x_arc_dep, x_arc_head)
            arc_logit = torch.squeeze(arc_logit, dim=3)

            x_rel_dep = torch.cat(x_all_dep_splits[self.arc_num:], dim=2)
            x_rel_head = torch.cat(x_all_head_splits[self.arc_num:], dim=2)

            rel_logit_cond = self.rel_biaffine2(x_rel_dep, x_rel_head)
